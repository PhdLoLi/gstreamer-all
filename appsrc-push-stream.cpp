/* GStreamer
 *
 * appsrc-push-stream.c: example for using appsrc in streaming mode.
 *
 * Copyright Modified from (C) 2008 Wim Taymans <wim.taymans@gmail.com>
 *
...
main (int argc, char *argv[])
{
  ...
  app->offset = 0;
//  std::cout <<"File length "<< app->length <<std::endl;
  ...
  std::string src = "appsrc://";
//  std::string src = argv[1];
  /* set to read from appsrc */
  if (gst_uri_is_valid (src.c_str()))
    uri = g_strdup (src.c_str());
  else
    uri = gst_filename_to_uri (src.c_str(), NULL);
