...
char *argv[]) 
{ 
    GstElement *convert, *muxer, *demuxer, *mqueue, *filesink, *queue3, *videorate; 
...
//    g_object_set (G_OBJECT (sink.audio), "sync", FALSE, NULL); 
    ...
    gst_bin_add_many (GST_BIN (pipeline),source.video, queue1.video, encoder.video, queue2.video,
      ...
      queue.video, queue.audio, parser.video, parser.audio, decoder.video, decoder.audio, sink.video, sink.audio, videorate, NULL); 

    gst_element_link_many (source.video, videorate, queue1.video, encoder.video, queue2.video, parser.video, decoder.video, queue.video, sink.video, NULL); 
    gst_element_link_many (source.audio, queue1.audio, convert, queue3, encoder.audio, queue2.audio, parser.audio, decoder.audio, queue.audio, sink.audio, NULL); 
